% Simulating 3-dimensional system with chemical reactions of type A+A -> 0
...
% position = numP by 3 array of dim=3D position of numP particles.
     ...
     for i=1:numP-1
        for j=i+1:numP
           if (position(i,1)==position(j,1))...
                           ...
                           && (position(i,3)==position(j,3))
                 position(j,:) = randi(L,1,3);
                 position(i,:) = randi(L,1,3);
...
jA = position(:,2);
kA = position(:,3);
create_particles(1,L,iA,jA,kA);
C = create_particles(2,L,iA,jA,kA);
...
for E = 1:numExperiments
    ...
    t=0;
      ...
      for T = 0:time_step:time_stop
         ...
         velocity = round(randn(updated_numP,dim)/3);
         ...
         % Kill meeted particles
           for i=1:updated_numP-1
               for j=i+1:updated_numP
                   if (updated_position(i,1)==updated_position(j,1)) && (updated_position(i,2)==updated_position(j,2))...
                           && (updated_position(i,3)==updated_position(j,3))                     
                      updated_position(j,:)=[];
                      updated_position(i,:)=[];
           ...
           kExp1A = updated_position(:,3);
           update_particles(iExp1A,jExp1A,kExp1A,C);           
...
function C = create_particles(num_fig,L,iExp1A,jExp1A,kExp1A)
   % Create the appropriate figure with axes and 3D box with grid
   subplot(2,2,num_fig)
   ...
   % Plot the particles
   C.particle_A = line(iExp1A,jExp1A,kExp1A,'LineStyle','none','marker','.',...
       'Color','b','markersize',30);
end

% Update and replot function(position,C)
function update_particles(iExp1A,jExp1A,kExp1A,C)         
   set(C.particle_A,'XData',iExp1A,'YData',jExp1A,'ZData',kExp1A)
   drawnow       
end
